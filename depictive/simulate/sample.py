...
def sample(k, Ncells=1000, n_doses=13, variances=None):
    ...
    s = _get_s_opt(k, variances, n_doses)
    ...
    for ws in range(s.size):
        ...
        y[:, ws] = _sample_labels(s[ws], k, x[:, :, ws])
    return [np.exp(s), x, y]
...
def _get_s_opt(k, variances, n_doses):
